...
// later HTTP request that needs to broadcast to it can land on two
// different function instances. Socket.IO's default in-memory adapter only
// tracks room membership within one process, so cross-instance broadcasts
// would silently go nowhere. The Postgres adapter moves that room/pub-sub
// state into Postgres (LISTEN/NOTIFY) instead, which every instance shares.
